...
import { Award, ExternalLink, Globe, BookOpen } from 'lucide-react';
...
export const ResearchPage: React.FC<ResearchPageProps> = ({ onNavigate }) => {
  ...
  const filteredGrants = GRANTS.filter(grant => {
    // Grants might have ranges like "2017 & 2018", so we check if the selected year is included in the string
    const matchesYear = filterYear === 'All Years' || grant.year.includes(filterYear);
    const matchesSearch = grant.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="py-8 animate-in fade-in duration-500">
      ...
      {/* Page Heading */}
      <div className="mb-8">
        <h1 className="text-[#0d141b] dark:text-white text-4xl font-black leading-tight tracking-tight mb-2">Research, Grants & Publications</h1>
        <p className="text-[#4c739a] dark:text-slate-400 text-lg">A comprehensive record of academic contributions, competitive funding, and research dissemination across global journals and conferences.</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-3/4 flex flex-col gap-8">
          ...
          {/* Chevening Award Section - Static Highlight */}
          <section className="mb-0">
            <div className="bg-gradient-to-br from-[#0a1f44] to-[#1e3a8a] rounded-2xl p-8 text-white shadow-xl relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start">
                    ...
                    <div className="flex-1">
                        <h2 className="text-2xl font-bold mb-2">Chevening Award Recipient</h2>
                        <p className="text-blue-100 text-base mb-6 font-light leading-relaxed">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center bg-white dark:bg-[#1a2632] p-4 rounded-xl shadow-sm border border-[#e7edf3] dark:border-slate-800 sticky top-20 z-40">
            <div className="relative flex-1 w-full">
                ...
                placeholder="Search grants, journals, or conferences..." 
                type="text"
                value={searchTerm}
          ...
          {/* Grants Section - Moved to Bottom */}
          <section className="mb-12 scroll-mt-20">
             <div className="flex items-center justify-between mb-4 px-2">
                <h3 className="text-xl font-bold flex items-center gap-2 text-[#0d141b] dark:text-white">
                    <Award className="text-amber-500" /> Honors & Competitive Grants
                </h3>
                <span className="text-sm text-[#4c739a]">{filteredGrants.length} Items</span>
             </div>
             
             <div className="bg-white dark:bg-[#1a2632] rounded-xl shadow-sm border border-[#e7edf3] dark:border-slate-800 divide-y divide-[#e7edf3] dark:divide-slate-800">
                {filteredGrants.map((grant, idx) => (
                    <div key={idx} className="p-6 hover:bg-amber-50 dark:hover:bg-amber-900/10 transition-colors cursor-default">
                        <div className="flex justify-between items-start gap-4 mb-2">
                            <h4 className="text-lg font-bold text-[#0d141b] dark:text-white leading-snug">{grant.organization}</h4>
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded uppercase shrink-0 bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300">
                                {grant.year}
                            </span>
                        </div>
                        <p className="text-sm text-[#4c739a] dark:text-slate-400 mb-3 leading-relaxed">
                            {grant.description}
                        </p>
                        ...
                        {grant.link && (
                            <div className="flex items-center gap-4">
                                <a 
                                    ...
                                    className="flex items-center gap-1 text-primary text-xs font-bold hover:underline"
                                >
                                    <span className="material-symbols-outlined text-sm">link</span> {grant.linkText || "View Details"}
                                </a>
                            </div>
                        )}
                    </div>
